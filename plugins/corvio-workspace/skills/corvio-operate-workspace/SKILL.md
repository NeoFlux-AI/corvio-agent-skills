...
This is Corvio Skill release `1.7.78`.
It implements `coding_agent_collaboration` contract version `2026-09-18.4`, compatibility family `coding-agent-collaboration-v1`, and
...
still requires cross-owner synthesis, a typed carrier, or source coverage that the Host cannot safely carry, delegate once to
`ask_corvio` with the exact current owner/source handles and the user's natural question. Read another owner in the Host only when that
missing fact can change the execution owner, target, scope, constraint, or host-native conclusion.
